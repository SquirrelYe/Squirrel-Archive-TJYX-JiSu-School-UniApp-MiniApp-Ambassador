<template>
  <view class="flow-box">
    <view class="item" :class="item.left == 1 ? 'left' : ''" :style="'top:' + item.top + 'px'" v-for="(item, index) in list" :key="index" <!-- #ifdef H5 -->
      ...
      @click="choose">
      <view class="pic">
        <image class="image" style="width: 100%; display: block" :src="item.image ? item.image : error"></image>
      </view>
      <view class="content">
        <text>{{ item.content }}</text>
        <view class="user">
          <image style="width: 40upx; height: 40upx; border-radius: 50%; margin-right: 10upx" :src="item.user.avatar ? item.user.avatar : error"></image>
          {{ item.user.name }}
        </view>
      </view>
    </view>
