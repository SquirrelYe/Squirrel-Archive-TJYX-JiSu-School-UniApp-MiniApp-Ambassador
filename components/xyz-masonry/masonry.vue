<template>
  <view class="masonry" style="">
    ...
    <view class="flex">
      <view>
        <image v-for="(item, index) in imgLeft" :key="index" :src="item.src" :mode="mode" :style="{ width: imgWidth + 'rpx' }" @click="goNavigate(item.id)" lazy-load></image>
      </view>
      <view>
        <image v-for="(item, index) in imgRight" :key="index" :src="item.src" :mode="mode" :style="{ width: imgWidth + 'rpx' }" @click="goNavigate(item.id)" lazy-load></image>
      </view>
    </view>
  </view>
</template>
