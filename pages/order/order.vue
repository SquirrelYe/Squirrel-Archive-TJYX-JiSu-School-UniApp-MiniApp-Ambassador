          ...
          <view v-for="(item, index) in tabItem.orderList" :key="index" class="order-item">
            ...
            <block v-if="tabCurrentIndex == 1">
              ...
              <view class="price-box" v-if="item.money">
                物品重量 <text class="weight">{{ item.weight }}kg</text> 订单价格 <text class="price">{{ item.money }}</text>
              </view>
              <view class="action-box b-t">
                <button class="action-btn" @click="call(item.location.phone)">联系同学</button>
...
export default {
  ...
  methods: {
    ...
    async updateLsend(item, condition) {
      ...
      // 询问
      if (!item.weight || !item.money) {
        this.$api.msg('请先填写重量价格信息~');
        return;
      }
      let res = await uni.showModal({
        title: '你确认要点嘛~',
    ...
    upStock(judge, add) {
      return new Promise(async (resolve, reject) => {
        // 结果返回值 -1 出现错误，1 处理正常
        const { data } = await this.$apis.stock.findByUserId(this.user.id).catch(err => {
          reject({ code: -1 });
        });
        const { id, money, score } = data;
        let m;
        // 依据全局config计算应该累加的金额
        (judge === 0 && (m = Number(money) + Number(add) * conf.log)) ||
          (judge === 1 && (m = Number(money) + Number(add) * conf.lsend)) ||
          (judge === 2 && (m = Number(money) + Number(add) * conf.card));
        // let m = Number(money) + Number(add)
        let final = await this.$apis.stock.updateMoneyScore(id, m, score).catch(err => {
          reject({ code: -1 });
        });
        console.log(data, final);
        if (data && final.data[0] === 1) {
          resolve({ code: 1 });
        }
        // 写入交易清单
        if (judge === 0);
