      ...
      <view class="cu-dialog" @tap.stop="">
        ...
        <view class="grid col-3 padding-sm">
          ...
          <view v-for="(item, index) in checkbox" class="padding-xs" :key="index">
            <button class="cu-btn orange lg block text-xs" :class="item.checked ? 'bg-orange' : 'line-orange'" @tap="ChooseMoney(item)" :data-value="item.value">
              {{ item.price }}元
...
export default {
  ...
  methods: {
    // 获取资金信息
    getStock() {
      this.$apis.stock.findByUserId(this.user.id).then(res => {
        console.log('账户资金信息', res.data);
        this.stock = res.data;
      });
    },
    // 获取优惠券信息
    ...
    async choose() {
      if (this.chooseMoney.length == 0) {
        this.$api.msg('没有选择提现金额喔~');
        return;
      }
      // 调用微信支付接口
      // 更新资产
      ...
      // 写入提现清单 减少资金账户
      let tx = await this.$apis.tixian.create(uid, aid, sid, 0, money, school_id).catch(err => {
        this.$api.msg('请求出错啦~');
        return;
      }); //u,a,s,c,m,sc 提现状态*（0.未处理、-1.拒绝、1.提现完成）
      // 统一扣款
      let m = this.stock.money - money;
      let score = this.stock.score;
      let final = await this.$apis.stock.updateMoneyScore(sid, m, score).catch(err => {
        this.$api.msg('请求出错啦~');
        return;
      });
    ...
    ChooseMoney(item) {
      if (item.price >= this.stock.money) {
        this.$api.msg('账户余额不足喔~');
        return;
      }
