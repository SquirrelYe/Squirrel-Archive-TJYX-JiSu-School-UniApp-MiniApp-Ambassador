...
export default {
  ...
  onShow() {
    console.log('onshow-->'), this.init(0);
  },
  onPageScroll(e) {
    //兼容iOS端下拉时顶部漂移
  ...
  methods: {
    ...
    async init(judge) {
      ...
      if (this.filterIndex === -1) {
        ...
        if (judge === 0) {
          uni.hideLoading();
          this.logisticList = res.data.rows;
        }
        if (judge === 1) {
          this.$api.msg('刷新成功');
    ...
    async confirm(judge, item) {
      ...
      if (res[1].confirm) {
        ...
        if (judge === -1) {
          ...
          if (log.data.condition !== 0) {
            this.$api.msg('订单已被其他人接单啦~');
            this.init(0);
            return;
          }
